<!-- Sidebar -->
<aside
    class="flex-shrink-0 bg-white border-r transition-all duration-300"
...
>
    ...
    <nav class="mt-4 px-2" x-data="{ openDropdown: '' }">
        <h3 x-show="sidebarOpen" class="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Main Menu</h3>

        <a href="{{ route('dashboard') }}" class="mt-2 flex items-center px-4 py-2 text-gray-700 rounded-md {{ request()->routeIs('dashboard') ? 'bg-gray-200' : '' }}">
        ...
        <div x-data="{ open: request()->routeIs('admin.products.*') }">
            <button @click="open = !open" class="w-full mt-2 flex items-center justify-between px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-md">
                ...
                <svg x-show="sidebarOpen" class="h-5 w-5 transform transition-transform" :class="{ 'rotate-180': open }" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>
            </button>
            <div x-show="open && sidebarOpen" x-collapse class="pl-8 py-2">
                <a href="{{ route('admin.products.index') }}" class="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-md">All Products</a>
                <a href="{{ route('admin.products.create') }}" class="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-md">Add New</a>
            </div>
        </div>

        <!-- Tautan Kategori -->
        <a href="{{ route('admin.categories.index') }}" class="mt-2 flex items-center px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-md {{ request()->routeIs('admin.categories.*') ? 'bg-gray-200' : '' }}">
        ...
        <!-- Tautan Brand -->
         <a href="{{ route('admin.brands.index') }}" class="mt-2 flex items-center px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-md {{ request()->routeIs('admin.brands.*') ? 'bg-gray-200' : '' }}">
        ...
        <!-- Tautan Order -->
        <a href="{{ route('admin.order') }}" class="mt-2 flex items-center px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-md {{ request()->routeIs('admin.order') ? 'bg-gray-200' : '' }}">
